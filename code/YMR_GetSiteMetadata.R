## YMR_GetSiteMetadata.R
# This script is intended to read in a CSV file of site locations at YMR and
...
df_sites <- read_csv(file.path(dir_sites, "YMR_SiteInfo.csv"))
sf_sites <- st_as_sf(df_sites, coords = c("long", "lat"), crs = st_crs(4326))

# load DEM, TWI, etc.
r_dem <- rast(file.path("results", "Youngmeyer_DEM_m.tif"))
r_twi <- rast(file.path("results", "Youngmeyer_TWI.tif"))
r_flowacc <- rast(file.path("results", "Youngmeyer_FlowAccumulation_cells.tif"))
r_slope <- rast(file.path("results", "Youngmeyer_Slope_degrees.tif"))
...
# save output
write_csv(df_sites, file.path(dir_sites, "YMR_SiteInfo_WithMetadata.csv"))
