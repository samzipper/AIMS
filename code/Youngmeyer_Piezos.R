...
reachnums_longitude <- c(555, 554, 553, 552, 551, 559, 575, 617) # get by looking at streams.shp in QGIS
pnts_longitude <- subset(pnts, 
                         StreamReach %in% reachnums_longitude & 
                           con_area_ha <= pnts_piezo$con_area_ha[pnts_piezo$Description == "ENM outlet"])
...
(p_map + p_dist) +
  ...
  ggsave(file.path("plots", "Youngmeyer_Piezos.png"),
...
# export
#Save map file
mapshot(m, file.path("docs", "Youngmeyer_Piezos.html"))
